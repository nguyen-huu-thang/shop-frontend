...
function Pagination({ currentPage, onPageChange }) {
    ...
    const totalPages = Math.ceil(specialProduct.length / productsPerPage); // Tính tổng số trang

    const handlePrev = () => {
        if (currentPage > 1) {
            onPageChange(currentPage - 1);
            navigate(`/storemanager/special?page=${currentPage - 1}`);
        }
    };
    ...
    const handlePageClick = (page) => {
        onPageChange(page);
        navigate(`/storemanager/special?page=${page}`);
    };
