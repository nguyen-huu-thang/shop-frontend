...
function Pagination({ currentPage, onPageChange }) {
    ...
    const totalPages = Math.ceil(bestSellProduct.length / productsPerPage); // Tính tổng số trang

    const handlePrev = () => {
        if (currentPage > 1) {
            onPageChange(currentPage - 1);
            navigate(`/storemanager/bestSell?page=${currentPage - 1}`);
        }
    };
    ...
    const handlePageClick = (page) => {
        onPageChange(page);
        navigate(`/storemanager/bestSell?page=${page}`);
    };
